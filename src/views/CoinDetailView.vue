    ...
    <template v-if="!isLoading">
          ...
          <div class="flex flex-row my-5">
            <label class="w-full" for="convertValue">
              <input
                ...
                type="number"
                :placeholder="
            !fromUsd ? `Valor en ${asset.symbol}` : `Valor en USD`"
      ...
      <table>
        <tr v-for="m in markets" :key="`${m.exchangeId}-${m.priceUsd}`" class="border-b">
          ...
          <td>
            <ButtonComponent v-if="!m.url" :isLoading="m.isLoading || false" @click="getWebsite(m)">
              <p v-show="!m.isLoading">
...
export default {
  ...
  computed: {
    ...
    min() {
      return Math.min(
        ...this.history.map(h => parseFloat(h.priceUsd).toFixed())
      )
    },
    max() {
      return Math.max(
